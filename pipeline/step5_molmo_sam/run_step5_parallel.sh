...
ROOT_DIR="${ROOT_DIR:-pipeline/step4_crop_images/seg_image_output/point_clipwithaffordance_output/$SPLIT}"
OUTPUT_ROOT="${OUTPUT_ROOT:-pipeline/step5_molmo_sam/molmo_output/$SPLIT}"
...
[ -d "$ROOT_DIR" ] || { echo "ERROR: input dir not found: $ROOT_DIR (in $REPO_ROOT)" >&2; exit 1; }
...
for shard in "${!GPU_ARR[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="$gpu" python "$SCRIPT" \
    ...
    --root_dir "$ROOT_DIR" --output_root "$OUTPUT_ROOT" \
