#!/usr/bin/env bash
# Source-frame DPT pixels + scalar source-confidence gate before point attention.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
export MODEL_CONFIG="${MODEL_CONFIG:-cdm_vggt_additive_stem_dpt_frozen}"
export EXP_NAME="${EXP_NAME:-source-vggt-dpt-additive-stem-frozen}"
